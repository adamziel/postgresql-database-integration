...
`wp-config.php`, and then sends you back to `wp-admin/install.php`.

Browser setup:

```text
https://example.test/wp-content/plugins/wordpress-databases-support/setup-database.php
```

The page presents SQLite, PostgreSQL, and DuckDB choices. It is intentionally
limited to first-install setup; if `wp-config.php` already exists, use the CLI
with `--force` instead.
...
After setup completes, open `wp-admin/install.php` and finish the normal
WordPress install.
