...
#[PHPUnit\Framework\Attributes\Group( 'duckdb-plugin' )]
class WP_DuckDB_DB_Adapter_Tests extends PHPUnit\Framework\TestCase {
	/**
	...
	public function test_db_copy_respects_explicit_duckdb_override(): void {
		...
		$this->assertSame( '0.1.0', $result['databases_support_dropin_version'] );
		$this->assertNull( $result['sqlite_dropin_version'] );
...
class WP_DuckDB_Plugin_SQL_Mode_Test_Driver extends WP_DuckDB_Driver {
	...
	public function get_connection(): WP_DuckDB_Connection {
		return new WP_DuckDB_Connection( array( 'duckdb' => new stdClass() ) );
	}
...
PHP;
	...
	private function get_wordpress_stub_code(): string {
		...
		return "<?php\n"
			...
			. "ini_set( 'error_log', " . var_export( $temp_dir . '/php-error.log', true ) . " );\n"
...
function wp_duckdb_plugin_test_state( $died ) {
	return array(
		...
		'databases_support_dropin_version' => defined( 'WORDPRESS_DATABASES_SUPPORT_DB_DROPIN_VERSION' ) ? WORDPRESS_DATABASES_SUPPORT_DB_DROPIN_VERSION : null,
